-- Migration: Sistema de Gestão de Instalações
...
CREATE TABLE IF NOT EXISTS public.historico_instalacoes (
  ...
  observacoes TEXT,
  dados_alteracao JSONB -- Para guardar dados adicionais (fotos, datas, etc)
);
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO public.historico_instalacoes (
      ...
      NEW.atualizado_por,
      CASE 
        WHEN NEW.status = 'ativo' THEN 'Instalação ativada'
...
-- Coordenadores podem ver histórico das suas instalações
CREATE POLICY "Coordenadores can view own historico"
  ...
  USING (
    ...
    EXISTS (
      ...
      JOIN grupos_campanha gc ON gc.id = i.grupo_id
      WHERE i.id = historico_instalacoes.instalacao_id
        AND gc.coordenador_id = auth.uid()
    )
  );
...
-- 12. Criar view para facilitar consultas
...
JOIN campanhas c ON c.id = i.campanha_id
JOIN enderecos e ON e.id = i.endereco_id
LEFT JOIN grupos_campanha gc ON gc.id = i.grupo_id
LEFT JOIN profiles p ON p.id = gc.coordenador_id;

-- RLS para a view
ALTER VIEW public.view_instalacoes_completa SET (security_invoker = true);

-- 13. Dados de exemplo para testes (comentado, descomentar se quiser)
-- UPDATE instalacoes 
-- SET 
